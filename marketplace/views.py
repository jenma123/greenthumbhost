...
def search(request):
    if not 'address' in request.GET:
        ...
        # get vendor ids that has the plant item the user is looking for
        fetch_vendors_by_plantitems = PlantItem.objects.filter(plant_title__icontains=keyword, is_available=True).values_list('vendor', flat=True)
        
        vendors = Vendor.objects.filter(Q(id__in=fetch_vendors_by_plantitems) | Q(vendor_name__icontains=keyword, is_approved=True, user__is_active=True))
